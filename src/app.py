...
from src.strategies import STRATEGY_SPECS
...
def render_backtest(cfg: dict, prices: pd.DataFrame) -> None:
    ...
    oos_rets = result.results.loc[result.results.index >= result.split_date, "daily_return"]
...
def render_sweep(cfg: dict) -> None:
    st.caption(
